...
int stun_calculate_hmac(buffer_type buf, std::size_t len, const buffer_type key, std::size_t keylen, buffer_type hmac, unsigned int *hmac_len, SHATYPE shatype)
{
	/*ERR_clear_error();
	UNUSED_ARG(shatype);
...
int stun_produce_integrity_key_str(buffer_type uname, buffer_type realm, buffer_type upwd, char key[64], SHATYPE shatype)
{
	//	ERR_clear_error();
	//	UNUSED_ARG(shatype);
	//
	...
	//	buffer_type str = (buffer_type)turn_malloc(sz + 1);
	//
...
static u32bits ns_crc32(const buffer_type buffer, u32bits len);

void print_hmac(const char *name, const void *s, size_t len);

/////////////////////////////////////////////////////////////////

int stun_get_command_message_len_str(buffer_type* buf, size_t len)
{
...
static int stun_set_command_message_len_str(buffer_type buf, int len) {
...
u16bits stun_get_method_str(const buffer_type buf, size_t len) {
...
u16bits stun_get_msg_type_str(const buffer_type buf, size_t len) {
	if (!buf || len < 2) return (u16bits)-1;
	return ((nswap16(((const u16bits*)buf)[0])) & 0x3FFF);
}

int is_channel_msg_str(const buffer_type buf, size_t blen) {
	return (buf && blen >= 4 && STUN_VALID_CHANNEL(nswap16(((const u16bits*)buf)[0])));
}

/////////////// message types /////////////////////////////////

int stun_is_command_message_str(const buffer_type buf, size_t blen)
{
	if (buf && blen >= STUN_HEADER_LENGTH) {
		if (!STUN_VALID_CHANNEL(nswap16(((const u16bits*)buf)[0]))) {
			if ((((char)buf[0]) & ((char)(0xC0))) == 0) {
...
int old_stun_is_command_message_str(const buffer_type buf, size_t blen, u32bits *cookie)
{
	if (buf && blen >= STUN_HEADER_LENGTH) {
		if (!STUN_VALID_CHANNEL(nswap16(((const u16bits*)buf)[0]))) {
			if ((((char)buf[0]) & ((char)(0xC0))) == 0) {
...
int stun_is_command_message_full_check_str(buffer_type buf, size_t blen, int must_check_fingerprint, int *fingerprint_present) {
	...
	u32bits crc32len = (u32bits)((((char*)fingerprint) - buf) - 4);
...
int stun_is_command_message_offset_str(const buffer_type buf, size_t blen, int offset) {
	return stun_is_command_message_str(buf + offset, blen);
}

int stun_is_request_str(const buffer_type buf, size_t len) {
	if (is_channel_msg_str(buf, len)) return 0;
	return IS_STUN_REQUEST(stun_get_msg_type_str(buf, len));
}

int stun_is_success_response_str(const buffer_type buf, size_t len) {
	if (is_channel_msg_str(buf, len)) return 0;
	return IS_STUN_SUCCESS_RESP(stun_get_msg_type_str(buf, len));
}

int stun_is_error_response_str(const buffer_type buf, size_t len, int *err_code, buffer_type err_msg, size_t err_msg_size) {
	if (is_channel_msg_str(buf, len)) return 0;
	if (IS_STUN_ERR_RESP(stun_get_msg_type_str(buf, len))) {
		if (err_code) {
			...
			if (sar) {
				if (stun_attr_get_len(sar) >= 4) {
					buffer_type* val = (buffer_type*)(stun_attr_get_value(sar));
...
int stun_is_challenge_response_str(const buffer_type buf, size_t len, int *err_code, buffer_type err_msg, size_t err_msg_size,
	buffer_type realm, buffer_type nonce, buffer_type server_name, int *oauth)
{
	...
	if (ret && (((*err_code) == 401) || ((*err_code) == 438))) {
		...
		if (sar) {
			...
			buffer_type* value = stun_attr_get_value(sar);
			if (value) {
...
int stun_is_response_str(const buffer_type buf, size_t len) {
	if (is_channel_msg_str(buf, len)) return 0;
...
int stun_is_indication_str(const buffer_type buf, size_t len) {
	if (is_channel_msg_str(buf, len)) return 0;
...
void stun_init_buffer_str(buffer_type buf, size_t *len) {
	*len = STUN_HEADER_LENGTH;
	ns_bzero(buf, *len);
}

void stun_init_command_str(u16bits message_type, buffer_type buf, size_t *len) {
...
void old_stun_init_command_str(u16bits message_type, buffer_type buf, size_t *len, u32bits cookie) {
	stun_init_buffer_str(buf, len);
	message_type &= (u16bits)(0x3FFF);
	((u16bits*)buf)[0] = nswap16(message_type);
	((u16bits*)buf)[1] = 0;
...
void stun_init_request_str(u16bits method, buffer_type buf, size_t *len) {
	stun_init_command_str(stun_make_request(method), buf, len);
}

void stun_init_indication_str(u16bits method, buffer_type buf, size_t *len) {
	stun_init_command_str(stun_make_indication(method), buf, len);
}

void stun_init_success_response_str(u16bits method, buffer_type buf, size_t *len, stun_tid* id) {
...
void old_stun_init_success_response_str(u16bits method, buffer_type buf, size_t *len, stun_tid* id, u32bits cookie) {
...
buffer_type* get_default_reason(int error_code)
{
	buffer_type* reason = (buffer_type*) "Unknown error";

	switch (error_code) {
	case 300:
		reason = (buffer_type*) "Try Alternate";
		break;
	case 400:
		reason = (buffer_type*) "Bad Request";
		break;
	case 401:
		reason = (buffer_type*) "Unauthorized";
		break;
	case 403:
		reason = (buffer_type*) "Forbidden";
		break;
	case 404:
		reason = (buffer_type*) "Not Found";
		break;
	case 420:
		reason = (buffer_type*) "Unknown Attribute";
		break;
	case 437:
		reason = (buffer_type*) "Allocation Mismatch";
		break;
	case 438:
		reason = (buffer_type*) "Stale Nonce";
		break;
	case 440:
		reason = (buffer_type*) "Address Family not Supported";
		break;
	case 441:
		reason = (buffer_type*) "Wrong Credentials";
		break;
	case 442:
		reason = (buffer_type*) "Unsupported Transport Protocol";
		break;
	case 443:
		reason = (buffer_type*) "Peer Address Family Mismatch";
		break;
	case 446:
		reason = (buffer_type*) "Connection Already Exists";
		break;
	case 447:
		reason = (buffer_type*) "Connection Timeout or Failure";
		break;
	case 486:
		reason = (buffer_type*) "Allocation Quota Reached";
		break;
	case 487:
		reason = (buffer_type*) "Role Conflict";
		break;
	case 500:
		reason = (buffer_type*) "Server Error";
		break;
	case 508:
		reason = (buffer_type*) "Insufficient Capacity";
		break;
	default:
		;
	};
...
static void stun_init_error_response_common_str(buffer_type buf, size_t *len, u16bits error_code, buffer_type* reason, stun_tid* id)
{
	...
	avalue[0] = 0;
	avalue[1] = 0;
	avalue[2] = (char)(error_code / 100);
	avalue[3] = (char)(error_code % 100);
	...
	stun_attr_add_str(buf, len, STUN_ATTRIBUTE_ERROR_CODE, (char*)avalue, alen);
	if (id) {
		stun_tid_message_cpy(buf, id);
	}
}

void old_stun_init_error_response_str(u16bits method, buffer_type buf, size_t *len, u16bits error_code, buffer_type* reason, stun_tid* id, u32bits cookie)
{
...
void stun_init_error_response_str(u16bits method, buffer_type buf, size_t *len,
	u16bits error_code, buffer_type* reason,
	stun_tid* id)
{
...
int stun_init_channel_message_str(u16bits chnumber, buffer_type buf, size_t *len, int length, int do_padding)
{
...
int stun_is_channel_message_str(const buffer_type buf, size_t *blen, u16bits* chnumber, int mandatory_padding)
{
...
int stun_get_message_len_str(buffer_type buf, size_t blen, int padding, size_t *app_len) {
	if (buf && blen) {
...
int stun_set_allocate_request_str(buffer_type buf, size_t *len, u32bits lifetime, int af4, int af6, char transport, int mobile, const char* rt, int ep) {
	...
	//LIFETIME
	{
		...
		if (stun_attr_add_str(buf, len, STUN_ATTRIBUTE_LIFETIME, (char*)(&field), sizeof(field)) < 0) return -1;
	}

	//MICE
	if (mobile) {
		if (stun_attr_add_str(buf, len, STUN_ATTRIBUTE_MOBILITY_TICKET, "", 0) < 0) return -1;
	}

	if (ep > -1) {
		uint8_t value = ep ? 0x80 : 0x00;
		if (stun_attr_add_str(buf, len, STUN_ATTRIBUTE_EVEN_PORT, (char*)&value, 1) < 0) return -1;
	}

	//RESERVATION-TOKEN, EVEN-PORT and DUAL-ALLOCATION are mutually exclusive:
	if (rt) {

		stun_attr_add_str(buf, len, STUN_ATTRIBUTE_RESERVATION_TOKEN, (char*)rt, 8);
		...
		if (af4 && !af6) {
			char field[4];
			field[0] = (char)STUN_ATTRIBUTE_REQUESTED_ADDRESS_FAMILY_VALUE_IPV4;
			field[1] = 0;
			field[2] = 0;
			field[3] = 0;
		...
		if (af6 && !af4) {
			char field[4];
			field[0] = (char)STUN_ATTRIBUTE_REQUESTED_ADDRESS_FAMILY_VALUE_IPV6;
			field[1] = 0;
			field[2] = 0;
			field[3] = 0;
			if (stun_attr_add_str(buf, len, STUN_ATTRIBUTE_REQUESTED_ADDRESS_FAMILY, field, sizeof(field)) < 0) return -1;
...
int stun_set_allocate_response_str(buffer_type buf, size_t *len, stun_tid* tid,
	const ioa_addr *relayed_addr1, const ioa_addr *relayed_addr2,
	const ioa_addr *reflexive_addr,
	u32bits lifetime, u32bits max_lifetime, int error_code, buffer_type* reason,
	...
	if (!error_code) {
		...
		if (reservation_token) {
			...
			stun_attr_add_str(buf, len, STUN_ATTRIBUTE_RESERVATION_TOKEN, (char*)(&reservation_token), 8);
		}
		...
		if (mobile_id && *mobile_id) {
			if (stun_attr_add_str(buf, len, STUN_ATTRIBUTE_MOBILITY_TICKET, (char*)mobile_id, strlen(mobile_id)) < 0) return -1;
		}
...
u16bits stun_set_channel_bind_request_str(buffer_type buf, size_t *len,
...
void stun_set_channel_bind_response_str(buffer_type buf, size_t *len, stun_tid* tid, int error_code, buffer_type* reason) {
	if (!error_code) {
...
void stun_set_binding_request_str(buffer_type buf, size_t *len) {
	stun_init_request_str(STUN_METHOD_BINDING, buf, len);
}

int stun_set_binding_response_str(buffer_type buf, size_t *len, stun_tid* tid,
	const ioa_addr *reflexive_addr, int error_code, buffer_type* reason,
...
int stun_is_binding_request_str(const buffer_type buf, size_t len, size_t offset)
{
	if (offset < len) {
		buf += offset;
		len -= offset;
...
int stun_is_binding_response_str(const buffer_type buf, size_t len) {
	if (stun_is_command_message_str(buf, len) &&
...
static void stun_tid_string_cpy(buffer_type s, const stun_tid* id) {
	if (s && id) {
		ns_bcopy((const void*)(id->tsx_id), s, STUN_TID_SIZE);
	}
}

static void stun_tid_from_string(const buffer_type s, stun_tid* id) {
	if (s && id) {
		ns_bcopy(s, (void*)(id->tsx_id), STUN_TID_SIZE);
	}
}

void stun_tid_from_message_str(const buffer_type buf, size_t len, stun_tid* id) {
	UNUSED_ARG(len);
...
int stun_get_requested_address_family(stun_attr_ref attr)
{
	if (attr) {
		...
		int val = ((const buffer_type)attr)[4];
		switch (val) {
...
u16bits stun_attr_get_channel_number(stun_attr_ref attr) {
	if (attr) {
		buffer_type* value = stun_attr_get_value(attr);
...
char stun_attr_get_even_port(stun_attr_ref attr) {
	if (attr) {
		buffer_type* value = stun_attr_get_value(attr);
		if (value) {
			if ((char)(value[0]) > 0x7F) return 1;
		}
	}
	return 0;
}

stun_attr_ref stun_attr_get_first_by_type_str(const buffer_type buf, size_t len, u16bits attr_type) {
...
stun_attr_ref stun_attr_get_first_str(buffer_type* buf, size_t len) {
...
stun_attr_ref stun_attr_get_next_str(buffer_type* buf, size_t len, stun_attr_ref prev) {

	if (!prev) return stun_attr_get_first_str(buf, len);
	else {
		buffer_type end = buf + stun_get_command_message_len_str(buf, len);
		int attrlen = stun_attr_get_len(prev);
		u16bits rem4 = ((u16bits)attrlen) & 0x0003;
		if (rem4) {
			attrlen = attrlen + 4 - (int)rem4;
		}
		const buffer_type attr_end = (buffer_type)prev + 4 + attrlen;
		if (attr_end < end) return attr_end;
		return NULL;
	}
}

int stun_attr_add_str(buffer_type buf, size_t *len, u16bits attr, char* avalue, int alen) {
	if (alen < 0) alen = 0;
	char tmp[1];
	if (!avalue) {
		alen = 0;
	...
	if (newlen >= MAX_STUN_MESSAGE_SIZE) return -1;
		...
		buffer_type attr_start = buf + clen;
...
int stun_attr_add_addr_str(buffer_type buf, size_t *len, u16bits attr_type, const ioa_addr* ca) {
	...
	if (stun_attr_add_str(buf, len, attr_type, (buffer_type)(&cfield), clen) < 0) return -1;

	return 0;
}

int stun_attr_get_addr_str(const buffer_type buf, size_t len, stun_attr_ref attr, ioa_addr* ca, const ioa_addr *default_addr) {
	...
	const buffer_type cfield = stun_attr_get_value(attr);
	if (!cfield) return -1;
...
int stun_attr_get_first_addr_str(const buffer_type buf, size_t len, u16bits attr_type, ioa_addr* ca, const ioa_addr *default_addr) {
...
int stun_attr_add_channel_number_str(buffer_type buf, size_t *len, u16bits chnumber) {

	u16bits field[2];
	field[0] = nswap16(chnumber);
	field[1] = 0;

	return stun_attr_add_str(buf, len, STUN_ATTRIBUTE_CHANNEL_NUMBER, (buffer_type)(field), sizeof(field));
}

int stun_attr_add_bandwidth_str(buffer_type buf, size_t *len, band_limit_t bps0) {

	u32bits bps = (band_limit_t)(bps0 >> 7);

	u32bits field = nswap32(bps);

	return stun_attr_add_str(buf, len, STUN_ATTRIBUTE_NEW_BANDWIDTH, (buffer_type)(&field), sizeof(field));
}

int stun_attr_add_address_error_code(buffer_type buf, size_t *len, int requested_address_family, int error_code)
{
	const buffer_type reason = get_default_reason(error_code);
	...
	stun_attr_add_str(buf, len, STUN_ATTRIBUTE_ADDRESS_ERROR_CODE, (buffer_type)avalue, alen);

	return 0;
}

int stun_attr_get_address_error_code(buffer_type buf, size_t len, int *requested_address_family, int *error_code)
{
	...
	if (buf && len) {
		...
		if (sar) {
			const buffer_type value = stun_attr_get_value(sar);
			if (!value) {
				return -1;
			}
			else {
...
u16bits stun_attr_get_first_channel_number_str(const buffer_type buf, size_t len) {
...
int stun_attr_add_fingerprint_str(buffer_type buf, size_t *len)
{
	u32bits crc32 = 0;
	stun_attr_add_str(buf, len, STUN_ATTRIBUTE_FINGERPRINT, (buffer_type)&crc32, 4);
...
int SASLprep(buffer_type s)
{
	if (s) {
		buffer_type strin = s;
		buffer_type strout = s;
		for (;;) {
			u08bits c = *strin;
			if (!c) {
				*strout = 0;
...
void print_bin_func(const char *name, size_t len, const void *s, const char *func)
{
	...
	size_t i;
	for (i = 0; i < len; i++) {
		printf("%02x", (int)((const buffer_type)s)[i]);
	}
	printf("]\n");
}

int stun_attr_add_integrity_str(turn_credential_type ct, buffer_type buf, size_t *len, hmackey_t key, password_t pwd, SHATYPE shatype)
{
...
int stun_attr_add_integrity_by_key_str(buffer_type buf, size_t *len, buffer_type uname, buffer_type realm, hmackey_t key, buffer_type nonce, SHATYPE shatype)
{
...
int stun_attr_add_integrity_by_user_str(buffer_type buf, size_t *len, buffer_type uname, buffer_type realm, buffer_type upwd, buffer_type nonce, SHATYPE shatype)
{
	hmackey_t key;
...
int stun_attr_add_integrity_by_user_short_term_str(buffer_type buf, size_t *len, buffer_type uname, password_t pwd, SHATYPE shatype)
{
	if (stun_attr_add_str(buf, len, STUN_ATTRIBUTE_USERNAME, uname, strlen((s08bits*)uname)) < 0)
		return -1;
 ...
 */
int stun_check_message_integrity_by_key_str(turn_credential_type ct, buffer_type buf, size_t len, hmackey_t key, password_t pwd, SHATYPE shatype)
{
	int res = 0;
	u08bits new_hmac[MAXSHASIZE];
	unsigned int shasize;
	const buffer_type old_hmac = NULL;
	...
	int new_len = ((const buffer_type)sar - buf) + 4 + shasize;
	if (new_len > orig_len)
		return -1;
 ...
 * Return -1 if failure, 0 if the integrity is not correct, 1 if OK
 */
int stun_check_message_integrity_str(turn_credential_type ct, buffer_type buf, size_t len, buffer_type uname, buffer_type realm, buffer_type upwd, SHATYPE shatype)
{
	hmackey_t key;
	password_t pwd;
...
int stun_attr_get_change_request_str(stun_attr_ref attr, int *change_ip, int *change_port)
{
	if (stun_attr_get_len(attr) == 4) {
		const buffer_type value = stun_attr_get_value(attr);
		if (value) {
...
int stun_attr_add_change_request_str(buffer_type buf, size_t *len, int change_ip, int change_port)
{
...
int stun_attr_add_response_port_str(buffer_type buf, size_t *len, u16bits port)
{
	u08bits avalue[4] = { 0,0,0,0 };
...
int stun_attr_add_padding_str(buffer_type buf, size_t *len, u16bits padding_len)
{
...
int encode_oauth_token_normal(const buffer_type server_name, encoded_oauth_token *etoken, const oauth_key *key, const oauth_token *dtoken);
int encode_oauth_token_normal(const buffer_type server_name, encoded_oauth_token *etoken, const oauth_key *key, const oauth_token *dtoken)
{
...
int decode_oauth_token_normal(const buffer_type server_name, const encoded_oauth_token *etoken, const oauth_key *key, oauth_token *dtoken);
int decode_oauth_token_normal(const buffer_type server_name, const encoded_oauth_token *etoken, const oauth_key *key, oauth_token *dtoken)
{
	UNUSED_ARG(server_name);
	UNUSED_ARG(etoken);
...
static int encode_oauth_token_gcm(const buffer_type server_name, encoded_oauth_token *etoken, const oauth_key *key, const oauth_token *dtoken, const buffer_type nonce0) {
...
static int decode_oauth_token_gcm(const buffer_type server_name, const encoded_oauth_token *etoken, const oauth_key *key, oauth_token *dtoken)
{
	if (server_name && etoken && key && dtoken) {
...
int encode_oauth_token(const buffer_type server_name, encoded_oauth_token *etoken, const oauth_key *key, const oauth_token *dtoken, const buffer_type nonce)
{
	UNUSED_ARG(nonce);
	if (server_name && etoken && key && dtoken) {
...
int decode_oauth_token(const buffer_type server_name, const encoded_oauth_token *etoken, const oauth_key *key, oauth_token *dtoken)
{
	if (server_name && etoken && key && dtoken) {
		switch (key->as_rs_alg) {
#if !defined(TURN_NO_GCM)
